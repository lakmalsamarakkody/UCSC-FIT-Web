...
<tr>
<td>
<a style="width:200px; text-align:center" href="{{ route('email.link.staff', ['email'=>$details['email'], 'token'=>$details['token'], 'role'=>$details['role']]) }}" class="button button-success" target="_blank" rel="noopener">Login</a>
</td>
</tr>
</table>
</td>
...
<p class="text-white" style="text-align: center; color: #fff !important;">OR <br> If you’re having trouble clicking the "Check Registration Status" button, copy and paste the URL below into your web browser: <br><a href="{{ route('email.link.staff', ['email'=>$details['email'], 'token'=>$details['token'], 'role'=>$details['role']]) }}">{{ route('email.link', ['email'=>$details['email'], 'token'=>$details['token'], 'role'=>$details['role']]) }}</a></p>
